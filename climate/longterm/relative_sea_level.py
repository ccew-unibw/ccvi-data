...
class CliLongtermRelativeSeaLevel(Indicator, NormalizationMixin):
    def __init__(
        self,
        config: ConfigParser,
        grid: GlobalBaseGrid,
        pillar: str = "CLI",
        dim: str = "longterm",
        id: str = "relative-sea-level",
    ):
        """Params defining indicator's place in index set to designed hierarchy by default"""
        self.event_data = DIVASubsidenceData(local=False, config=config)
...
if __name__ == "__main__":
    ...
    indicator = CliLongtermRelativeSeaLevel(config=config, grid=grid)
    indicator.run()

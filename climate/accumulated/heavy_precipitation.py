...
from base.datasets.gee_heavy_precipitation import GEEHeavyPrecipitationData
...
class CliAccumulatedHeavyPrecipitation(Indicator, NormalizationMixin):
    def __init__(
        self,
        config: ConfigParser,
        grid: GlobalBaseGrid,
        pillar: str = "CLI",
        dim: str = "accumulated",
        id: str = "heavy-precipitation",
    ):
        """Params defining indicator's place in index set to designed hierarchy by default"""
        self.event_data = GEEHeavyPrecipitationData(local=False, config=config)
...
if __name__ == "__main__":
    ...
    indicator = CliAccumulatedHeavyPrecipitation(config=config, grid=grid)
    indicator.run()

...
from conflict.level.surrounding import ConLevelSurrounding
from conflict.shared import NormalizationMixin, PersistenceMixin


class ConPersistenceSurrounding(Indicator, NormalizationMixin, PersistenceMixin):
    def __init__(
        self,
        config: ConfigParser,
        grid: GlobalBaseGrid,
        base_indicator: ConLevelSurrounding,
        pillar: str = "CON",
        dim: str = "persistence",
        id: str = "surrounding",
    ):
    ...
    def load_data(self) -> pd.DataFrame:
        ...
        self.validate_indicator_input(self.base_indicator, ConLevelSurrounding)
...
if __name__ == "__main__":
    ...
    base_indicator = ConLevelSurrounding(config=config, grid=grid)
    indicator = ConPersistenceSurrounding(config=config, grid=grid, base_indicator=base_indicator)
    indicator.run()
